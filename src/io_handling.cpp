...
size_t FileReader::readArgument(const std::string &str) const {
    ...
    if (splits.size() != 2) {
        logger.log(FATAL, "Invalid format: " + str);
    }
...
size_t FileReader::strToSize_t(const std::string &str) const {
    try {
        ...
        logger.log(FATAL, "Invalid argument: " + std::string(e.what()));
    } catch (const std::out_of_range& e) {
        logger.log(FATAL, "Out of range: " + std::string(e.what()));
    }
    return 0;
}
...
void FileReader::ParseHouse(std::ifstream &file, std::shared_ptr<House> house) const {
    if (!file.is_open()) {
        logger.log(FATAL, "error reading house from file");
    }
    ...
    while (std::getline(file, line) && row_index < num_of_rows) {
        ...
        for (size_t col_index = 0; col_index < width; col_index++) {
            ...
            if (c == 'D') {
                ...
                logger.log(FATAL, std::format("Invalid charecter in house map ({},{})", row_index, col_index));
            }
        }
        row_index++;
    }
...
FileReader::file_reader_output FileReader::readFile() const {
    ...
    if (!file.is_open()) {
        logger.log(FATAL, "Failed to open the file: " + file_path);
    }

    std::string line;

    if (!std::getline(file, line)) {
        logger.log(FATAL, "Failed to read first line of input file");
    }

    if (std::getline(file, line)) {
        max_num_of_steps = readArgument(line);
    } else {
        logger.log(FATAL, "Failed to read 2nd line with MaxSteps");
    }

    if (std::getline(file, line)) {
        max_battery_steps = readArgument(line);
    } else {
        logger.log(FATAL, "Failed to read 3rd line with MaxBattery");
    }

    if (std::getline(file, line)) {
        rows_count = readArgument(line);
    } else {
        logger.log(FATAL, "Failed to read 4th line with Rows");
    }

    if (std::getline(file, line)) {
        cols_count = readArgument(line);
    } else {
        logger.log(FATAL, "Failed to read 5th line with Cols");
    }
